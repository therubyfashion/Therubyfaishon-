import{b as M,A as W,r as d,f as O,t as i,j as e,m as _,J as q,g as N,c as v,d as f,i as C,h as D,E as R}from"./index-CKnaIYP3.js";import{A as G}from"./arrow-left-Cmddlpk6.js";import{C as J}from"./circle-alert-2GGIePdo.js";import{M as K}from"./mail-D9JJuhB4.js";import{R as F}from"./refresh-cw-Cu4haHFy.js";import{C as Q}from"./circle-check-mvXfNMV_.js";function re(){const x=M(),k=W(),[A,y]=d.useState(!1),[P,b]=d.useState(!1),[h,$]=d.useState(null),[w,E]=d.useState(""),[m,B]=d.useState(""),[g,S]=d.useState(["","","","","",""]),[T,V]=d.useState(null);d.useEffect(()=>{const s=new URLSearchParams(k.search),o=s.get("email"),t=s.get("uid"),n=s.get("message");o&&E(o),t&&B(t),n&&V(n),(async()=>{try{const l=await N(v(f,"settings"));l.empty||$(l.docs[0].data())}catch(l){l.code==="resource-exhausted"?console.warn("VerifyPrompt: Firestore quota exceeded."):console.error("Error fetching settings:",l)}})();const r=O.onAuthStateChanged(l=>{!l&&t&&(i.error("Please sign in to verify your email."),x("/login"))});return()=>r()},[k,x]);const U=(s,o)=>{if(o.length>1&&(o=o.slice(-1)),!/^\d*$/.test(o))return;const t=[...g];if(t[s]=o,S(t),o&&s<5){const n=document.getElementById(`otp-${s+1}`);n==null||n.focus()}},L=(s,o)=>{if(o.key==="Backspace"&&!g[s]&&s>0){const t=document.getElementById(`otp-${s-1}`);t==null||t.focus()}},H=async()=>{var o;const s=g.join("");if(s.length!==6){i.error("Please enter the full 6-digit code.");return}b(!0);try{const t=h?Promise.resolve(h):N(v(f,"settings")).then(p=>p.empty?null:p.docs[0].data()),n=C(f,"users",m),a=D(n),[r,l]=await Promise.all([t,a]);if(!l.exists()){i.error("User not found."),b(!1);return}const c=l.data();if(c.emailOtp===s){await R(n,{isVerified:!0,emailOtp:null});const p=`
          <!DOCTYPE html>
          <html>
          <head>
            <meta charset="utf-8">
